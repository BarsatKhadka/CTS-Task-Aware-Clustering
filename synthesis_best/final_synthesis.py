...
Combines best approaches for all three targets:
  ...
  POWER: XGB(v11 params) on base_pw(58d) + timing(18d) = 76 dims
         Zero-shot MAPE = 32.0%, K=10 shot MAPE = 9.8% ← ≤10% TARGET (v20: median k_hat)
         (K-shot calibration: k_hat = median(actual/pred) on K support samples)

  WL: LGB+Ridge blend (α=0.3) on base_wl(53d)+gravity(19d)+extra_scale(3d)+net(9d)=84d
      Zero-shot MAPE = 7.0% (v21: net features reduce AES WL 24.9%→12.8%, mean 11%→7%)
...
Requires caches:
  ...
  net_features_cache.pkl      (per-net RSMT, HPWL, RUDY — for WL)
"""
...
def build_all_features(df_in, dc, sc_cache, tc, skc, gc, ec, nc=None):
    """
    ...
    for _, row in df_in.iterrows():
        ...
        net_feats = [
            ...
            float(nf.get('net_hpwl_mean', 0.0) or 0.0),
            np.log1p(float(nf.get('net_hpwl_p90', 0.0) or 0.0)),
            float(nf.get('frac_high_fanout', 0.0) or 0.0),
            float(nf.get('rudy_mean', 0.0) or 0.0),
            float(nf.get('rudy_p90', 0.0) or 0.0),
            rsmt_t * cd / max(n_ff * die_area, 1.0),
            float(nf.get('rudy_mean', 0.0) or 0.0) * cd,
        ]  # 9 dims
        rows_wl.append(base_wl + gravity + extra_scale + net_feats)  # 84 dims
...
def kshot_calibrate(pred_pw, actual_pw, pred_wl, actual_wl, K=20, n_reps=200, seed=42):
    ...
    for _ in range(n_reps):
        ...
        k_pw = np.clip(np.median(actual_pw[supp] / pred_pw[supp]), 0.1, 10.0)
        k_wl = np.clip(np.median(actual_wl[supp] / pred_wl[supp]), 0.1, 10.0)
...
if __name__ == '__main__':
    ...
    kshot_results = {K: {} for K in [0, 1, 3, 5, 10, 20]}
    for held in designs:
        tr = meta_df['design_name'] != held
        te = meta_df['design_name'] == held
        sc_pw = StandardScaler()
    ...
    pw_10 = np.mean([kshot_results[10].get(d,(0,0))[0] for d in designs])
    wl_10 = np.mean([kshot_results[10].get(d,(0,0))[1] for d in designs])
    print(f"\n  K=10 Calibrated (median k_hat):")
    print(f"    Power: {pw_10:.1f}% MAPE {'✓' if pw_10<=10 else '✗'}")
    print(f"    WL:    {wl_10:.1f}% MAPE")
    print(f"    Skew:  {mean_sk:.4f} MAE (unchanged — no calibration needed)")

    print(f"\n  Per-design:")
    print(f"  {'Design':>10} | {'Pw ZS':>7} | {'WL ZS':>7} | {'Sk ZS':>8} | {'Pw K10':>8}")
    print(f"  {'-'*10}-+-{'-'*7}-+-{'-'*7}-+-{'-'*8}-+-{'-'*8}")
    for d in designs:
        r = results[d]
        pw10 = kshot_results[10].get(d, (0,0))[0]
        sk_mark = '✓' if r['skew'] < 0.10 else '✗'
        pw_mark = '✓' if pw10 <= 10.0 else '✗'
        print(f"  {d:>10} | {r['power']:>6.1f}% | {r['wl']:>6.1f}% | "
              f"{r['skew']:>7.4f}{sk_mark} | {pw10:>7.1f}%{pw_mark}")
