"""
Zero-shot absolute predictor v12: Spatial Heatmap + kNN + Gravity + Power Fix.
...
SPATIAL_CACHE_V12  = f'{BASE}/absolute_v12_spatial_cache.pkl'
...
# -----------------------------------------------------------------------
# SPATIAL FEATURES (FF density heatmap + kNN distances)
# -----------------------------------------------------------------------

def compute_spatial_features(def_path, clock_port='clk'):
    """
    Spatial FF distribution features from DEF, separate parse (no NETS needed).

    Computes:
    - 4×4 FF density grid (16 dims, normalized to die dimensions) — GAN-CTS style
    - Grid statistics: entropy, gini, h/v asymmetry, center vs periphery
    - kNN distances (normalized by sqrt(die_area)) — captures local FF clustering

    All features normalized to die → in-distribution for OOD designs like zipdiv.
    """
    ...
    cell_pos = {m.group(1): (float(m.group(2))/units, float(m.group(3))/units)
                for m in re.finditer(comp_pat, content)}
    ff_pos = np.array([(cell_pos[ff][0], cell_pos[ff][1]) for ff in ff_names if ff in cell_pos])
    if len(ff_pos) < 4:
        return {}

    # Normalize to [0,1]²
    fx = (ff_pos[:, 0] - x0) / (die_w + 1e-9)
    fy = (ff_pos[:, 1] - y0) / (die_h + 1e-9)

    # 4×4 density grid
    G = 4
    grid = np.zeros(G * G)
    gx = np.clip((fx * G).astype(int), 0, G-1)
    gy = np.clip((fy * G).astype(int), 0, G-1)
    np.add.at(grid, gx * G + gy, 1)
    grid /= (grid.sum() + 1e-9)

    p = grid + 1e-9; p /= p.sum()
    entropy = float(-np.sum(p * np.log(p + 1e-9)))
    gini_g  = float(_gini(grid))

    # Spatial imbalance
    h_asym = float(abs((fx < 0.5).mean() - (fx >= 0.5).mean()))
    v_asym = float(abs((fy < 0.5).mean() - (fy >= 0.5).mean()))

    # Center vs periphery fraction
    center_frac = float(((fx > 0.25) & (fx < 0.75) & (fy > 0.25) & (fy < 0.75)).mean())
    periph_frac = float(((fx < 0.1) | (fx > 0.9) | (fy < 0.1) | (fy > 0.9)).mean())

    # kNN distances (absolute µm, then normalized by die_scale)
    k_max = min(9, len(ff_pos))
    tree = cKDTree(ff_pos)
    dists, _ = tree.query(ff_pos, k=k_max)
    knn1 = dists[:, min(1, k_max-1)] / (die_scale + 1e-9)
    knn4 = dists[:, min(4, k_max-1)] / (die_scale + 1e-9)
    knn8 = dists[:, min(8, k_max-1)] / (die_scale + 1e-9)
    ...
    feats = {
        ...
        'sp_h_asym':      h_asym,
        'sp_v_asym':      v_asym,
        'sp_center_frac': center_frac,
        'sp_periph_frac': periph_frac,
        'sp_quad_max':    float(grid.max()),
        'sp_knn1_mean':   float(knn1.mean()),
        'sp_knn1_std':    float(knn1.std()),
        'sp_knn4_mean':   float(knn4.mean()),
        'sp_knn4_std':    float(knn4.std()),
        'sp_knn8_mean':   float(knn8.mean()),
    }
    # Full 4×4 grid (16 dims — GAN-CTS spatial distribution)
    for i, v in enumerate(grid):
        feats[f'sp_g{i:02d}'] = float(v)
    return feats
...
def build_caches(df):
    ...
    # Build gravity + timing degree cache (v10/v11)
    ...
    # Build spatial cache (v12: FF density grid + kNN distances)
    if os.path.exists(SPATIAL_CACHE_V12):
        with open(SPATIAL_CACHE_V12, 'rb') as f: sp_cache = pickle.load(f)
        missing_sp = [p for p in pids if p not in sp_cache]
    else:
        sp_cache = {}
        missing_sp = list(pids)

    if missing_sp:
        print(f"Computing spatial (grid+kNN) features for {len(missing_sp)} placements...")
        for i, pid in enumerate(missing_sp):
            if pid not in dc:
                continue
            row = df[df['placement_id'] == pid].iloc[0]
            design = row['design_name']
            def_path = row['def_path'].replace('../dataset_with_def/placement_files', PLACEMENT_DIR)
            clock_port = CLOCK_PORTS.get(design, 'clk')
            sp_cache[pid] = compute_spatial_features(def_path, clock_port)
            if (i + 1) % 100 == 0:
                print(f"  {i+1}/{len(missing_sp)}")

        with open(SPATIAL_CACHE_V12, 'wb') as f:
            pickle.dump(sp_cache, f)
        print(f"Saved spatial cache: {len(sp_cache)} entries")

    return dc, sc, tc, gc, sp_cache
...
def build_features(df, dc, sc, tc, gc, sp_cache=None):
    rows_pw, rows_wl = [], []
    y_pw_r, y_wl_r = [], []
    meta = []
    if sp_cache is None:
    ...
    for _, row in df.iterrows():
        ...
        graph_feats = extra_scale + [
            ...
            # === V12: SPATIAL HEATMAP + kNN (FF density grid, GAN-CTS style) ===
            # All normalized to die dims → in-distribution for OOD designs
            spf.get('sp_entropy', 0.0),
            spf.get('sp_gini', 0.0),
            spf.get('sp_h_asym', 0.0),
            spf.get('sp_v_asym', 0.0),
            spf.get('sp_center_frac', 0.0),
            spf.get('sp_periph_frac', 0.0),
            spf.get('sp_quad_max', 0.0),
            spf.get('sp_knn1_mean', 0.0),
            spf.get('sp_knn1_std', 0.0),
            spf.get('sp_knn4_mean', 0.0),
            spf.get('sp_knn4_std', 0.0),
            spf.get('sp_knn8_mean', 0.0),
        ] + [spf.get(f'sp_g{i:02d}', 0.0) for i in range(16)]  # 4×4 grid = 16 dims
...
def zipdiv_eval(df_train, df_test, dc, sc_cache, tc, gc, pw_cls, pw_kw, wl_alpha=0.5, name="", sp_cache=None):
    X_pw_tr, X_wl_tr, y_pw_tr, y_wl_tr, meta_tr = build_features(df_train, dc, sc_cache, tc, gc, sp_cache)
    X_pw_te, X_wl_te, _, _, meta_te = build_features(df_test, dc, sc_cache, tc, gc, sp_cache)
    if len(meta_te) == 0:
...
def main():
    print("=" * 70)
    print("Zero-Shot Absolute Predictor v12 — Spatial Heatmap + kNN + Gravity")
    print("=" * 70)
    print("v12 adds to v11:")
    print("  - 4×4 FF density grid (GAN-CTS style, normalized to die)")
    print("  - 7 spatial summary stats (entropy, gini, h/v asym, center, periph, quad_max)")
    print("  - kNN FF distances (kNN-1/4/8 mean+std, normalized to die scale)")
    print("  All in-distribution for zipdiv (die-normalized) ✓")
    print()
    ...
    dc, sc_cache, tc, gc, sp_cache = build_caches(df_all)
    X_pw, X_wl, y_pw, y_wl, meta_df = build_features(df_train, dc, sc_cache, tc, gc, sp_cache)
    X_pw_te, X_wl_te, _, _, meta_te = build_features(df_test, dc, sc_cache, tc, gc, sp_cache)

    xgb_pw = dict(n_estimators=300, max_depth=4, learning_rate=0.05,
                  subsample=0.8, colsample_bytree=0.8, random_state=42, verbosity=0)

    # --- LODO: Power + WL key alpha values ---
    print("--- LODO: f_lgb=0.0 (Ridge-only, best zipdiv) ---")
    lodo_eval(X_pw, X_wl, y_pw, y_wl, meta_df, XGBRegressor, xgb_pw,
              wl_alpha=0.0, name="v12_ridge")

    print("--- LODO: f_lgb=0.3 (best LODO in v11) ---")
    lodo_eval(X_pw, X_wl, y_pw, y_wl, meta_df, XGBRegressor, xgb_pw,
              wl_alpha=0.3, name="v12_blend")
    ...
    zipdiv_eval(df_train, df_test, dc, sc_cache, tc, gc, XGBRegressor, xgb_pw,
                wl_alpha=0.0, name="v12_ridge", sp_cache=sp_cache)
    print("\n--- Zipdiv: f_lgb=0.3 ---")
    zipdiv_eval(df_train, df_test, dc, sc_cache, tc, gc, XGBRegressor, xgb_pw,
                wl_alpha=0.3, name="v12_blend", sp_cache=sp_cache)
