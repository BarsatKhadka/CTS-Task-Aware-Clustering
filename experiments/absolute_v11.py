"""
Zero-shot absolute predictor v11: WIRE-GRAPH MESSAGE PASSING (Gravity Vectors) + Power Fix.
...
def build_features(df, dc, sc, tc, gc):
    ...
    for _, row in df.iterrows():
        ...
        rows_pw.append(base + timing)    # no graph_feats for power
...
def main():
    ...
    # --- LODO WL alpha sweep (WL now includes timing features) ---
    print("--- LODO WL alpha sweep (WL includes timing features) ---")
    ...
    zipdiv_eval(df_train, df_test, dc, sc_cache, tc, gc, XGBRegressor, xgb_pw,
                wl_alpha=0.0, name="ridge_only")
    print("\n--- Zipdiv: f_lgb=0.3 ---")
    zipdiv_eval(df_train, df_test, dc, sc_cache, tc, gc, XGBRegressor, xgb_pw,
                wl_alpha=0.3, name="blend_0.3")
