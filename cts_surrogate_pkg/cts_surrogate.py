...
# PKG_DIR = directory containing this file (cts_surrogate_pkg/)
PKG_DIR = os.path.dirname(os.path.abspath(__file__))
# BASE = project root (one level up), kept for backward-compat with cache paths
...
class CTSSurrogate:
    ...
    @classmethod
    def from_package(cls, pkg_dir: str = None, model: str = '4head') -> 'CTSSurrogate':
        """
        One-line loader — discovers models and caches from the package directory.

        Parameters
        ----------
        pkg_dir : str, optional
            Path to the cts_surrogate_pkg/ directory.
            Defaults to the directory containing this file.
        model : '4head' (default) or '3head'
            '4head' predicts power + WL + skew + hold_vio
            '3head' predicts power + WL + skew only
        ...
        """
        d = pkg_dir or PKG_DIR

        def _pkl(name):
            path = os.path.join(d, 'caches', name)
            with open(path, 'rb') as f:
                return pickle.load(f)

        model_file = 'cts_4head.pkl' if model == '4head' else 'cts_3head.pkl'
        model_path = os.path.join(d, 'models', model_file)

        return cls.load_with_caches(
            model_path,
            def_cache     = _pkl('def_cache.pkl'),
            saif_cache    = _pkl('saif_cache.pkl'),
            timing_cache  = _pkl('timing_cache.pkl'),
            skew_cache    = _pkl('skew_cache.pkl'),
            gravity_cache = _pkl('gravity_cache.pkl'),
            net_cache     = _pkl('net_cache.pkl'),
        )
...
if __name__ == '__main__':
    ...
    MANIFEST = os.path.join(PKG_DIR, 'data', 'manifest.csv')
    EXPLOG   = os.path.join(PKG_DIR, 'data', 'zipdiv_gt.csv')
    PDIR     = os.path.join(os.path.dirname(PKG_DIR),
                            'dataset_with_def', 'placement_files')
